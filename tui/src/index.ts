...
客户端内置命令（在对话中使用）：
  ...
  /mode web/forward 表示切换直连还是使用 web 模式
  /sessions      列出所有历史会话
  /session <id>  加载某个历史会话
  /system <文本> 设置系统提示词
...
const initialMode = modeArg === 'web' || modeArg === 'forward'
  ? modeArg
  : coreRuntime.defaults.mode;
...
const config: ClientConfig = {
  baseUrl: getArg('--url') ?? coreRuntime.api.baseUrl ?? `http://localhost:${defaultPort}`,
  model: getArg('--model') ?? coreRuntime.defaults.model ?? 'gpt-4o',
  system: systemPrompt,
  tools,
  timeoutMs: getArg('--timeout') ? parseInt(getArg('--timeout')!, 10) * 1000 : coreRuntime.api.timeoutMs,
  sessionId: getArg('--session-id') ?? buildDefaultSessionId(),
  traceEnabled: hasFlag('--trace') || hasFlag('--no-trace')
    ? traceEnabled
    : coreRuntime.defaults.traceEnabled,
  stream: hasFlag('--stream') || hasFlag('--no-stream')
    ? streamEnabled
    : coreRuntime.defaults.stream,
  tracePreviewChars: Number.isFinite(previewChars) && previewChars > 0 ? previewChars : coreRuntime.defaults.tracePreviewChars,
